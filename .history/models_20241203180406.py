...
class Book(BaseModel):
    ...
    year_published: conint(ge=1450, le=2024, title="Year Published")  # Ensuring the year is realistic
    isbn: constr(regex=r'^(97(8|9))?\d{9}(\d|X)$', title="ISBN")  # Validating ISBN format (ISBN-10 or ISBN-13)
...
class BookUpdate(BaseModel):
    ...
    year_published: Optional[conint(ge=1450, le=2024, title="Year Published")] = None
    isbn: Optional[constr(regex=r'^(97(8|9))?\d{9}(\d|X)$', title="ISBN")] = None
